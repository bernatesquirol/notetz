import ReactFlow, { Controls, Background, Handle, NodeTypes } from 'reactflow';
...
const nodeWidth = 172;
const nodeHeight = 36;
const styleNodes:any = {float:'left',border: "1px solid black", borderRadius: 5, width: nodeWidth, height: nodeHeight, textAlign:'center'}
// import levenshtein from 'fast-levenshtein'
const ScaleNode = memo(({id, data, targetPosition, sourcePosition}:any)=>{
  return <>
        <Handle type="target" position={targetPosition} />
        <div style={styleNodes} onClick={()=>data?.setContext({key: data.root, scale:data.scale})}>
          {data.root?`${data.root} `:''}{data.scale}
        </div>
...
const ModeNode = memo(({id, data, targetPosition, sourcePosition}:any)=>{
  return <>
        <Handle type="target" position={targetPosition} />
        <div style={{...styleNodes, width:nodeWidth/2}} onClick={()=>data?.setContext({key: data.root, scale:data.scale})}>
          {data.root?`${data.root} `:''}{data.scale}
        </div>
        <div style={{...styleNodes, width:nodeWidth/2}} onClick={()=>data?.setContext({key: Note.transpose(data.root, data.interval), scale:data.scale})}>
          {data.root?`${Note.transpose(data.root, data.interval)} `:''}{data.scale}
        </div>
        <Handle type="source" position={sourcePosition} />
  </>
})
...
export const FlowSelectorContext = autoContextProviderFactory<{mode?:string, key?: string}>({mode:undefined, key: undefined}, 'FlowSelector')
const FlowSelector = ({width, height})=>{
  const {scale, key, setContext} = FlowSelectorContext.useFlowSelectorContext()
  const createNode = useCallback((n:{data?:any}&Record<string,any>)=>{
    return {...n, 
  ...
  const {nodes, edges} = useMemo(()=>{
    ...
    let nodesRaw = [...children, ...parents, scale].map((k)=>(createNode({id:k, data:{scale:k, root: key}, type:  'scaleNode'})))
    ...
    let edgesRaw = [...edgesChildren, ...edgesParents]
    ...
    let modes = Scale.modeNames(scale).reduce((acc:any,[interval,mode]:any)=>{
    ...
    let nodemode = nodes.find(({id})=>id===scale)
    // north is S i S is N
    let N = modes.N
    N.sort(({intMode:intModeA}, {intMode:intModeB})=>intModeA-intModeB)
    let nodesNorth = N.map((p:any,i)=>(createNode({
      id: p.mode, 
      type: 'modeNode',
      data:{ scale: p.mode, root: key, interval: p.interval },
      position:{x:nodemode.position.x, y:nodemode.position.y+nodeHeight*1.2*(i+1)}
    })))
    let S =  modes.S
    S.sort(({intMode:intModeA}, {intMode:intModeB})=>intModeB-intModeA)
    let nodesSouth = S.map((p,i)=>(createNode({
      id: p.mode, 
      type: 'modeNode',
      data:{scale: p.mode, root: key, interval: p.interval},
      position:{x:nodemode.position.x, y:nodemode.position.y-nodeHeight*1.2*(i+1)}
